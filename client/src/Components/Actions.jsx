...
const Actions = ({ post: post_ }) => {
  ...
  // !handle Like And UnLikePost:
  const handleLikeAndUnLikePost = async () => {
    if (!currentUser)
      return showToast("Error", "Please login to like post", "error");
    if (isLiking) return;
    setIsLiking(true);
    try {
      const res = await fetch("/api/v1/posts/likeunlikepost/" + post._id, {
        method: "PUT",
        headers: {
  ...
  const handleReplyPost = async () => {
    if (!currentUser)
      return showToast("Error", "Please login to reply post", "error");
    if (isReplying) return;
    setIsReplying(true);
    try {
      const res = await fetch("/api/v1/posts/replypost/" + post._id, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
